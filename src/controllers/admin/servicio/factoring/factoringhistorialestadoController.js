...
export const updateFactoringhistorialestado = async (req, res) => {
  logger.debug(line(), "controller::updateFactoringhistorialestado");
  const { id } = req.params;
  ...
  logger.debug(line(), "factoringhistorialestadoValidated:", factoringhistorialestadoValidated);
  ...
  try {
    ...
    if (!factoringhistorialestado) {
      logger.warn(line(), "Factoringhistorialestado no existe: [" + factoringhistorialestadoValidated.factoringhistorialestadoid + "]");
    ...
    logger.debug(line(), "factoringhistorialestadoUpdated:", factoringhistorialestadoUpdated.dataValues);
...
export const getFactoringhistorialestadosByFactoringid = async (req, res) => {
  logger.debug(line(), "controller::getFactoringhistorialestadosByFactoringid");
  ...
  try {
    ...
    if (!factoring) {
      logger.warn(line(), "Factoring no existe: [" + factoringhistorialestadoValidated.factoringid + "]");
      throw new ClientError("Datos no válidos", 404);
    }
...
export const createFactoringhistorialestado = async (req, res) => {
  logger.debug(line(), "controller::createFactoringhistorialestado");
  ...
  try {
    ...
    if (!factoringestado) {
      logger.warn(line(), "Factoring estado no existe: [" + factoringhistorialestadoValidated.factoringestadoid + "]");
      throw new ClientError("Datos no válidos", 404);
    }
    ...
    if (factoringhistorialestadoValidated.archivos) {
      for (const archivoid of factoringhistorialestadoValidated.archivos) {
        ...
        if (!archivo) {
          logger.warn(line(), "Archivo no existe: [" + archivoid + "]");
          throw new ClientError("Datos no válidos", 404);
        }
    ...
    for (const archivo of archivos) {
      ...
      logger.debug(line(), "archivofactoringhistorialestadoCreated:", archivofactoringhistorialestadoCreated.dataValues);
    }
...
export const activateFactoringhistorialestado = async (req, res) => {
  logger.debug(line(), "controller::activateFactoringhistorialestado");
  const { id } = req.params;
  const factoringhistorialestadoSchema = yup
    .object()
    .shape({
  ...
  try {
    ...
    logger.debug(line(), "factoringhistorialestadoActivated:", factoringhistorialestadoActivated);
...
export const deleteFactoringhistorialestado = async (req, res) => {
  logger.debug(line(), "controller::deleteFactoringhistorialestado");
  const { id } = req.params;
  const factoringhistorialestadoSchema = yup
    .object()
    .shape({
  ...
  try {
    ...
    logger.debug(line(), "factoringhistorialestadoDeleted:", factoringhistorialestadoDeleted);
...
export const getFactoringhistorialestados = async (req, res) => {
  logger.debug(line(), "controller::getFactoringhistorialestados");
  //logger.info(line(),req.session_user.usuario._idusuario);
  ...
  try {
    ...
    var factoringhistorialestadosJson = jsonUtils.sequelizeToJSON(factoringhistorialestados);
    //logger.info(line(),factoringhistorialestadoObfuscated);
