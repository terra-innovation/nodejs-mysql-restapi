...
export const updateInversionistacuentabancariaOnlyAliasAndCuentaBancariaEstado = async (req, res) => {
  logger.debug(line(), "controller::updateInversionistacuentabancariaOnlyAliasAndCuentaBancariaEstado");
  const { id } = req.params;
  ...
  try {
    ...
    if (!inversionistacuentabancariaestado) {
      logger.warn(line(), "Cuenta bancaria estado no existe: [" + inversionistacuentabancariaValidated.cuentabancariaestadoid + "]");
    ...
    if (!inversionistacuentabancaria) {
      logger.warn(line(), "Inversionista cuenta bancaria no existe: [" + inversionistacuentabancariaValidated.inversionistacuentabancariaid + "]");
      throw new ClientError("Datos no válidos", 404);
    }
    ...
    if (!cuentabancaria) {
      logger.warn(line(), "Cuenta bancaria no existe: [" + inversionistacuentabancaria._idcuentabancaria + "]");
      throw new ClientError("Datos no válidos", 404);
    }
...
export const getInversionistacuentabancarias = async (req, res) => {
  logger.debug(line(), "controller::getInversionistacuentabancarias");
...
export const activateInversionistacuentabancaria = async (req, res) => {
  logger.debug(line(), "controller::activateInversionistacuentabancaria");
  const { id } = req.params;
...
export const deleteInversionistacuentabancaria = async (req, res) => {
  logger.debug(line(), "controller::deleteInversionistacuentabancaria");
  const { id } = req.params;
  const inversionistacuentabancariaSchema = yup
    .object()
    .shape({
...
export const getInversionistacuentabancariaMaster = async (req, res) => {
  logger.debug(line(), "controller::getInversionistacuentabancariaMaster");
  const transaction = await sequelizeFT.transaction();
  try {
...
export const createInversionistacuentabancaria = async (req, res) => {
  logger.debug(line(), "controller::createInversionistacuentabancaria");
  ...
  try {
    ...
    if (!inversionista) {
      logger.warn(line(), "Inversionista no existe: [" + inversionistacuentabancariaValidated.inversionistaid + "]");
      throw new ClientError("Datos no válidos", 404);
    }

    var banco = await bancoDao.findBancoPk(transaction, inversionistacuentabancariaValidated.bancoid);
    if (!banco) {
      logger.warn(line(), "Banco no existe: [" + inversionistacuentabancariaValidated.bancoid + "]");
      throw new ClientError("Datos no válidos", 404);
    }

    var cuentatipo = await cuentatipoDao.findCuentatipoPk(transaction, inversionistacuentabancariaValidated.cuentatipoid);
    if (!cuentatipo) {
      logger.warn(line(), "Cuenta tipo no existe: [" + inversionistacuentabancariaValidated.cuentatipoid + "]");
      throw new ClientError("Datos no válidos", 404);
    }
    var moneda = await monedaDao.findMonedaPk(transaction, inversionistacuentabancariaValidated.monedaid);
    if (!moneda) {
      logger.warn(line(), "Moneda no existe: [" + inversionistacuentabancariaValidated.monedaid + "]");
      throw new ClientError("Datos no válidos", 404);
    }
    ...
    if (cuentasbancarias_por_numero && cuentasbancarias_por_numero.length > 0) {
      logger.warn(line(), "El número de cuenta [" + inversionistacuentabancariaValidated.numero + "] se encuentra registrado. Ingrese un número de cuenta diferente.");
    ...
    if (cuentasbancarias_por_alias && cuentasbancarias_por_alias.length > 0) {
      logger.warn(line(), "El alias [" + inversionistacuentabancariaValidated.alias + "] se encuentra registrado. Ingrese un alias diferente.");
    ...
    var camposCuentaBancariaFk = {};
    camposCuentaBancariaFk._idinversionista = inversionista._idinversionista;
    ...
    var camposCuentaBancariaAdicionales = {};
    camposCuentaBancariaAdicionales.inversionistacuentabancariaid = uuidv4();
    ...
    var camposInversionistaCuentaBancariaCreate = {};
    ...
    logger.debug(line(), "inversionistacuentabancariaCreated:", inversionistacuentabancariaCreated.dataValues);
