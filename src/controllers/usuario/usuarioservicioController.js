...
import * as archivocolaboradorDao from "../../daos/archivocolaboradorDao.js";
...
export const suscribirUsuarioServicio = async (req, res) => {
  ...
  const empresa = await empresaDao.getEmpresaByRuc(req, usuarioservicioValidated.ruc);
  ...
  const paisSede = await paisDao.findPaisPk(req, usuarioservicioValidated.paissedeid);
  if (!paisSede) {
    logger.warn(line(), "País de sede no existe: [" + usuarioservicioValidated.paissedeid + "]");
    throw new ClientError("Datos no válidos", 404);
  }
  const distritoSede = await distritoDao.getDistritoByDistritoid(req, usuarioservicioValidated.distritosedeid);
  if (!distritoSede) {
    logger.warn(line(), "Distrito de sede no existe: [" + usuarioservicioValidated.distritosedeid + "]");
    throw new ClientError("Datos no válidos", 404);
  }

  const banco = await bancoDao.findBancoPk(req, usuarioservicioValidated.bancoid);
  if (!banco) {
    logger.warn(line(), "Banco no existe: [" + usuarioservicioValidated.bancoid + "]");
    throw new ClientError("Datos no válidos", 404);
  }

  const cuentatipo = await cuentatipoDao.findCuentatipoPk(req, usuarioservicioValidated.cuentatipoid);
  if (!cuentatipo) {
    logger.warn(line(), "Cuenta tipo no existe: [" + usuarioservicioValidated.cuentatipoid + "]");
    throw new ClientError("Datos no válidos", 404);
  }

  const moneda = await monedaDao.findMonedaPk(req, usuarioservicioValidated.monedaid);
  if (!moneda) {
    logger.warn(line(), "Moneda no existe: [" + usuarioservicioValidated.monedaid + "]");
    throw new ClientError("Datos no válidos", 404);
  }

  const cuentasbancarias_por_numero = await cuentabancariaDao.getCuentasbancariasByIdbancoAndNumero(req, banco._idbanco, usuarioservicioValidated.numero, filter_estado);
  ...
  const usuarioservicioestado = await usuarioservicioestadoDao.getUsuarioservicioestadoByIdusuarioservicioestado(req, usuarioservicioaestado_en_revision);
  ...
  const usuarioConected = await usuarioDao.getUsuarioByIdusuario(req, usuarioservicioValidated._idusuario);
  const personaConected = await personaDao.getPersonaByIdusuario(req, usuarioservicioValidated._idusuario);
  ...
  const ficharucCreated = await crearArchivoFichaRuc(req, usuarioservicioValidated, empresaCreated);
  logger.debug(line(), "ficharucCreated:", ficharucCreated);

  const reportetributarioCreated = await crearArchivoReporteTributarioParaTerceros(req, usuarioservicioValidated, empresaCreated);
  logger.debug(line(), "reportetributarioCreated:", reportetributarioCreated);

  const vigenciapoderCreated = await crearArchivoVigenciaPoderRepresentanteLegal(req, usuarioservicioValidated, colaboradorCreated);
  logger.debug(line(), "vigenciapoderCreated:", vigenciapoderCreated);

  const encabezadocuentabancariaCreated = await crearArchivoEncabezadoCuentaBancaria(req, usuarioservicioValidated, cuentabancariaCreated);
  ...
  const servicioempresaverificacionCreated = await servicioempresaverificacionDao.insertServicioempresaverificacion(req, camposServicioempresaverificacionCreate);
  ...
  const usuarioservicioverificacionCreated = await usuarioservicioverificacionDao.insertUsuarioservicioverificacion(req, camposUsuarioservicioverificacionCreate);
...
const crearArchivoEncabezadoCuentaBancaria = async (req, usuarioservicioValidated, cuentabancariaCreated) => {
  //Copiamos el archivo
  ...
  await archivocuentabancariaDao.insertArchivoCuentaBancaria(req, {
...
const crearArchivoVigenciaPoderRepresentanteLegal = async (req, usuarioservicioValidated, colaboradorCreated) => {
  //Copiamos el archivo
  ...
  await archivocolaboradorDao.insertArchivoColaborador(req, {
    _idarchivo: archivoCreated._idarchivo,
...
const crearArchivoReporteTributarioParaTerceros = async (req, usuarioservicioValidated, empresaCreated) => {
  //Copiamos el archivo
...
const crearArchivoFichaRuc = async (req, usuarioservicioValidated, empresaCreated) => {
  //Copiamos el archivo
