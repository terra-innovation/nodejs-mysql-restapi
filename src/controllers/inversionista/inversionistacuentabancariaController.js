...
export const createInversionistacuentabancaria = async (req, res) => {
  ...
  try {
    ...
    if (!inversionistacuentabancariaAllowed) {
      logger.warn(line(), "Inversionista no asociado al usuario: [" + inversionistacuentabancariaValidated._idempresa + ", " + _idusuario_session + "]");
      throw new ClientError("Datos no válidos", 404);
    }
    ...
    camposEmpresaCuentaBancariaCreate._idinversionista = inversionista._idinversionista;
    camposEmpresaCuentaBancariaCreate._idcuentabancaria = cuentabancariaCreated._idcuentabancaria;
    camposEmpresaCuentaBancariaCreate.inversionistacuentabancariaid = uuidv4();
...
export const updateInversionistacuentabancariaOnlyAlias = async (req, res) => {
  logger.debug(line(), "controller::updateInversionistacuentabancariaOnlyAlias");
  const { id } = req.params;
  ...
  try {
    ...
    if (!inversionistacuentabancariaAllowed) {
      logger.warn(line(), "Inversionista no asociado al usuario: [" + inversionistacuentabancaria._idempresa + ", " + _idusuario_session + "]");
      throw new ClientError("Datos no válidos", 404);
    }
...
export const getInversionistacuentabancariaMaster = async (req, res) => {
  ...
  try {
    ...
    cuentasbancariasMaster.inversionista = inversionista;
