...
export const simulateFactoringLogicV2 = async (
  ...
  try {
    var constante_igv: Partial<ConfiguracionAppCreationAttributes> = await configuracionappDao.getIGV(transaction);
    var constante_costo_cavali: Partial<ConfiguracionAppCreationAttributes> = await configuracionappDao.getCostoCAVALI(transaction);
    var constante_comison_bcp: Partial<ConfiguracionAppCreationAttributes> = await configuracionappDao.getComisionBCP(transaction);
    ...
    var financiero_concepto_comisionft = await financieroconceptoDao.getComisionFinanzaTech(transaction);
    var financiero_concepto_cavali = await financieroconceptoDao.getCostoCAVALI(transaction);
    var financiero_concepto_transaccion = await financieroconceptoDao.getCostoTransaccion(transaction);
    ...
    let comision_ft: Record<string, any> = {
